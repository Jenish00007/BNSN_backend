...
exports.getFavorites = catchAsyncErrors(async (req, res, next) => {
  ...
  const favorites = await Favorite.find({ user: userId }).populate({
    path: "product",
    // Return the full product document so that favourites have
    // access to seller/shop information, userId, category details, etc.
    // Also populate category & subcategory names for UI consistency.
    populate: [
      {
        path: "category",
        select: "name",
